...
- 本 ADR のレビュー・承認。
- ~~PoC（elm-pages ビルド出力を Tauri v2 の最小構成で起動し、静的 JSON 取得まで動作確認）に着手。~~
  → **着手済み。** Tauri v2 の最小構成一式を `app/src-tauri/` に追加。
  セットアップ・起動・検証手順は [`app/src-tauri/README.md`](../../app/src-tauri/README.md) を参照。
  なお PoC 作成環境には WebView / Tauri CLI / elm ツールチェーンが無く、GUI 起動の実行検証は
  メンテナのローカル（nix）環境で行う。
