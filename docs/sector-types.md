...
### 提案 8：ハードコードされた既定比率の削除 ★実施済み

提案 7 で比率の算出が 1 か所（`Config.ratiosOver`）に集まったことで、記録が無いときの
ために持っていた比率表が二重の真実になった。削除して「算出できないなら均等割り」に
一本化する。

削除したもの:

- `Circuit.sectorDefaultRatio`（`1 / 3`）
- `LeMans.defaultRatio` / `defaultWeight` / `totalDefaultWeight`
  — 15 ミニセクター分の手書きの重み表（SCLB 26、Z12 24、… FL 2、合計 150）

置き換え後は、記録の合計が 0 のとき `1 / toFloat (List.length order)` を返すだけ。
セクターなら 1/3、ミニセクターなら 1/15 で、**両粒度が同じ規則になり、どちらも
「自分の部分がどれくらいの長さか」の表を持たなくてよくなった**。`Circuit` 側の
`1 / 3` も、3 分割の均等割りをたまたま定数で書いていただけだったことになる。

#### 挙動が変わる点

**記録が 1 つも無い間のトラック形状が変わる。** ル・マンの既定比率は重み付きで
S1 0.18 / S2 0.367 / S3 0.453 だったが、均等割りでは **S1 0.2 / S2 0.267 /
S3 0.533**（3 / 4 / 8 個の均等割り）になる。

これは見た目にはっきり出る。というのも 2.2 の調査で分かったとおり、**本番データでは
ミニセクタータイムが 1 つも届いていない**（`Data.Wec.Laps` が `miniSectors = Nothing`
を直書きしている）ため、ル・マンのトラックは常にこの既定比率で描かれているから。
つまり **VRT スナップショット（`lap-180.png`）の更新が必要**。

この点は、ローダーの結線（別途対応予定の「提案 A」）が入ればミニセクターの実測値が
使われるようになり、既定比率の出番は「データ読み込み直後の一瞬」だけに戻る。
...
| 7 | 提案 8（既定比率表の削除） | 二重の真実を消す。均等割りに一本化 | `Circuit`, `Circuit/LeMans`, `Tracker/Config` | 実施済み |
