...
- **ベストタイム更新。** summary の `index.bestTimeChanges` には出ていて timeline には無い。
  ...
  **適用済み**: ファステストラップだけを、首位と同じくクラスごとに `fastestLap` として出す
  ...
  ドライバーを添え、どちらもアプリがその車の周回から引く。1周目は集計から外した。
  1周目はセクター1だけが後の周より短い区間で計測されている: Fuji 2024・2025、Spa 2025、
  Bahrain 2024 ではトップの1周目のS1が後のどのS1より0.6〜1.7秒速く、Fuji 2025 では36台中
  10台が1周目に自己ベストのS1を出す一方、1周目のS2が後の周を上回った車は全14ラウンドで
  0台(S1〜S3の合計は1周目の周回タイムと一致する)。Le Mans(3年とも)・COTA 2025・
  São Paulo 2026 では1周目のS1は逆に大きく遅い(セーフティカー先導・雨のスタート)。
  **適用済み**: `Round.Index` の記録(`bestTimeChanges`、アプリの色付けの基準)も
  ファステストラップとS1から1周目を外した。S2・S3・ミニセクター(Le Mans 2025 のみ。
  1周目が後の周を上回った区間は無い)は1周目を含めたまま。これで Fuji 2025 のファステスト
  ラップは 1:29.690(1周目)から 1:30.507(#12、2周目)になる。車ごとの自己ベスト
  (アプリ側で周回から計算)は1周目を含めたまま(未対応)。
...
1. **caution エピソード(`slowZone` / `fullCourseYellow` の開始・終了)を追加** —
   ...
   **適用済み**: 全車の通過をコントロールラインが見た順に並べ、旗が前の通過と変わった
   通過ごとに1件出す(`fullCourseYellow` / `safetyCar` / `redFlag` / `greenFlag`、
   車番なし)。レースは緑で始まる扱いで、セーフティカー先導のスタートは最初の通過で
   `safetyCar` になる。FF(チェッカー)は各車の `finished` なので旗としては数えない。
   `SF` はスローゾーンではなくセーフティカーと読んだ: SF の通過はどのラウンドでもほぼ全車に
   あり、周回タイムが大きく落ちる(Le Mans 2025 で平均394秒、GF は238秒)。局所のスロー
   ゾーンならこうはならない。通過順に並べた旗は GF と SF が入り混じらず、切り替わりはきれいに
   1回ずつ取れる(FCY→SF や SF→RF→SF もそのまま1件ずつ)。Le Mans 2025 は FCY 11回・SC 1回・
   Green 12回で +24行。時刻は旗が最初にラインで読まれた通過なので、実際の宣言からは、その後
   最初にどれかの車がラインを越えるまでの分だけ遅れる(数十台が走っていれば通常は数秒)。
