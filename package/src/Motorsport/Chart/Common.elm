...
renderLine :
    Scales
    -> { color : Color, emphasis : Emphasis, label : String, points : List ( Int, Int ) }
    -> Svg msg
renderLine scales { color, emphasis, label, points } =
    ...
    g []
        [ Path.element linePath
            [ SvgAttr.stroke (Color.toCss strokeValue)
...
terminalMarker : Scales -> { color : Color, label : String } -> ( Int, Int ) -> Svg msg
...
terminalLabel : { x : Float, y : Float, color : Color, label : String } -> Svg msg
