...
小さいものも同時に直した: エラー文言、モジュール配置、そして 1 ラウンドあたり
5 回走っていた読み取りを 1 回にして各リーダーへ渡す形（この構造は B-3 取り消し後も残る）。

### B-3 を取り消した理由

読み手が無かった。

- **`canonical_id` はどこからも読まれていなかった。** B-3 の看板だった
  Mercedes / Mercedes-AMG の解決が一度も起きておらず、export はフィードの綴りを
  そのまま書き、アプリは相変わらず車番から色を生成していた。そしてこの問題を今日
  直す一番安い方法は `manufacturers.json` に 1 行足すことで、DB は要らない。
- **チーム横断のクエリも存在しない。**
- **容量は 14.28 → 14.28MB。**新テーブルが `cars` の節約をちょうど相殺していた。

B-2（ドライバー）は 2.7MB 減り、308 人の同一性という答えられなかった問いを開いた。
B-3 は同じ形をしていて、そのどちらも無かった。将来クエリが要るようになったときに
`Db.Drivers` と同じ手順でやればよく、そのときは読み手がある。

`Db.Numbering` の抽出も B-3 に属していたので `Db.Drivers` に畳み戻した。使い手が
1 つになった共有モジュールと、通過するだけのシムが 2 本残っていた。
