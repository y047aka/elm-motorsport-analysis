...
|------------|------|-------------|
| `swap` | Component | Container for the swap functionality |
| `swap-on` | Part | Element visible when checkbox is checked or swap is active |
| `swap-off` | Part | Element visible when checkbox is unchecked or swap is inactive |
| `swap-indeterminate` | Part | Element visible when checkbox is in indeterminate state |
...
- **Required**: Controlled by checkbox state - `swap-on` shown when checked, `swap-off` when unchecked
- **Recommended**: Use `swap-rotate` or `swap-flip` for animation effects
